                    ...
                    <?php if($model->Appraisal_Status == 'Supervisor_Level' && $model->isSupervisor()): ?>
                         ...
                         <?= Html::a('<i class="fas fa-backward"></i> To Appraisee.',['probationbacktoappraisee','appraisalNo'=> $model->Appraisal_No,'employeeNo' => $model->Employee_No],
                            ...
                            <?= Html::a('<i class="fas fa-forward"></i> To Overview ',['submitprobationtooverview','appraisalNo'=> $model->Appraisal_No,'employeeNo' => $model->Employee_No],
                                ...
                                'class' => 'mx-1 btn btn-app submitforapproval','data' => [
                                'confirm' => 'Are you sure you want to Submit PIP Appraisal to Overview Manager ?',
                                'method' => 'post',
                            ],
                                'title' => 'Submit PIP to Overview Manager.'
                            ]) ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel" style="position: absolute">PIP Appraisal</h4>
            </div>
...
$script = <<<JS
    ...
    /*Reject Appraisal Submission by Appraisee - rejectappraiseesubmition*/
     ...
     $('#confirmation').hide();
     $('#pip-over_view_manager_comments').change(function(e){
        const Comments = e.target.value;
        const Appraisal_No = $('#pip-appraisal_no').val();
        if(Appraisal_No.length){
            
            const url = $('input[name=url]').val()+'pip/setfield?field=Over_View_Manager_Comments';
            $.post(url,{'Over_View_Manager_Comments': Comments,'Appraisal_No': Appraisal_No}).done(function(msg){
                   ...
                   $('#pip-key').val(msg.Key);
                    ...
                    console.table(msg);
                    if((typeof msg) === 'string') { // A string is an error
                        const parent = document.querySelector('.field-pip-over_view_manager_comments');
                        const helpbBlock = parent.children[2];
                        helpbBlock.innerText = msg;
