...
$script = <<<JS
   ...
   // $('.timepicker').timepicker();
    ...
    //Submit form and get results in json    
        ...
        $('#timesheetline-start_time').on('change', function(e){
            e.preventDefault();
                  
            const Line_No = $('#timesheetline-line_no').val();
            const Start_Time = $('#timesheetline-start_time').val();
            const Date = $('#timesheetline-date').val();
            
            
            const url = $('input[name="absolute"]').val()+'timesheetline/setstarttime';
            $.post(url,{'Start_Time': Start_Time,'Line_No': Line_No,'Date': Date}).done(function(msg){
                   //populate empty form fields with new data
                    console.log(typeof msg);
                    console.table(msg);
                    if((typeof msg) === 'string') { // A string is an error
                        const parent = document.querySelector('.field-timesheetline-start_time');
        ...
        /*Commit End Time */
         ...
         $('#timesheetline-end_time').on('change', function(e){
            e.preventDefault();
                  
            const Line_No = $('#timesheetline-line_no').val();
            const End_Time = $('#timesheetline-end_time').val();
            
            
            const url = $('input[name="absolute"]').val()+'timesheetline/setendtime';
            $.post(url,{'End_Time': End_Time,'Line_No': Line_No}).done(function(msg){
                   //populate empty form fields with new data
                    console.log(typeof msg);
                    console.table(msg);
                    if((typeof msg) === 'string') { // A string is an error
                        const parent = document.querySelector('.field-timesheetline-end_time');
                        const helpbBlock = parent.children[2];
                        helpbBlock.innerText = msg;
                        disableSubmit();
                    }else{ // An object represents correct details
                    ...
                    $('#timesheetline-key').val(msg.Key);
                    $('#timesheetline-hours_worked').val(msg.Hours_Worked);
