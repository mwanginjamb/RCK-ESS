...
class RfqController extends Controller
{
    public function behaviors()
    {
        return [
            ...
            'contentNegotiator' =>[
                ...
                'only' => ['list','list-approved','list-pending'],
                'formatParam' => '_format',
                'formats' => [
    ...
    public function actionCreate(){
       // Yii::$app->recruitment->printrr($this->getPayrollscales());
        $model = new Timesheet();
        $service = Yii::$app->params['ServiceName']['TsCard'];

        /*Do initial request */
        if(!isset(Yii::$app->request->post()['Timesheet'])){
        ...
        if(Yii::$app->request->post() && Yii::$app->navhelper->loadpost(Yii::$app->request->post()['Timesheet'],$model) ){
        ...
        $model->Period_Month = ($model->Period_Month == '0001-01-01')?date('Y-m-d'):$model->Period_Month;
    ...
    public function actionUpdate($No){
        ...
        $model->Period_Month = ($model->Period_Month == '0001-01-01')?date('Y-m-d'):$model->Period_Month;
        // Yii::$app->recruitment->printrr($model->Period_Month);
   ...
   // Get list
    ...
    public function actionList(){
        $service = Yii::$app->params['ServiceName']['SentQuotationList'];
        $filter = [
            'Status' => '',
        ];
        ...
        foreach($results as $item){
            ...
            if(!empty($item->No ))
            {
                ...
                $Viewlink = Html::a('<i class="fas fa-eye"></i>',['view' ],[
                    'class'=>'btn btn-outline-primary btn-xs','title' => 'View Request.',
                    'data' => [
                        'params' => [
                            'RFQ_No' => $item->No,
                            'Committee_Member_ID' => Yii::$app->user->identity->{'User ID'}
                        ],
                        'method' => 'POST'
                    ]
                 ]);
                ...
                $result['data'][] = [
                    ...
                    'Requisiton_No' => !empty($item->Requisiton_No)?$item->Requisiton_No:'',
                    'Current_Budget' => !empty($item->Current_Budget)?$item->Current_Budget:'',
                    'Supplier_Category' => !empty($item->Supplier_Category)?$item->Supplier_Category:'',
                    'Action' => $Viewlink ,
    ...
    public function actionListApproved(){
        $service = Yii::$app->params['ServiceName']['TsApproved'];
        $filter = [
    ...
    public function actionListPending(){
        $service = Yii::$app->params['ServiceName']['TsPendingApproval'];
        $filter = [
            'Employee_No' => Yii::$app->user->identity->{'Employee No_'},
        ];
